    ...
    window.fetch = async (input, init) => {
        ...
        // Ventas (fase 3/4): regalos y paquetes, quién quiere subir de plan, Directoras invitadas
            ...
            if (method === 'PATCH') response = respond({ id: path.split('/')[2], contacted_at: path.endsWith('/contacted') ? iso(0) : null, fulfilled_at: path.endsWith('/fulfilled') ? iso(0) : null })
                ...
                { id: 'g1', kind: 'gift', quantity: 1, user: demoUser('Blanca Silvia Novoa Macías', 'EJ-0001', 'Plan Ejecutivo'), person: { id: 'np-x', name: 'Ana Valadez Ángeles', consultant_code: 'EJ-0101' }, plan: { id: 'ps', name: 'Plan Standard' }, requested_at: iso(0), contacted_at: null, fulfilled_at: null, contacted_by: null },
                { id: 'g2', kind: 'unit_package', quantity: 12, user: demoUser('Julieta Ruiz Maldonado', 'EJ-0002', 'Plan Nacional'), person: null, plan: { id: 'ps', name: 'Plan Standard' }, requested_at: iso(1440), contacted_at: iso(0), fulfilled_at: null, contacted_by: { id: 'a', name: 'David' } },
            ]))
            else if (path === '/plan-interests') response = respond(page([
                { id: 'i1', user: demoUser('Carla Domínguez', 'EJ-0003', 'Plan Básico'), plan: { id: 'pe', name: 'Plan Ejecutivo', color: '#4E31C0' }, current_plan: { id: 'pb', name: 'Plan Básico' }, signals: { lock: 3, sheet: 1, request: 1 }, last_feature: 'unity:early', requested_at: iso(0), contacted_at: null, updated_at: iso(0) },
                { id: 'i2', user: demoUser('Rosa Luz Arteaga', 'EJ-0004', 'Plan Básico'), plan: { id: 'pe', name: 'Plan Ejecutivo', color: '#4E31C0' }, current_plan: { id: 'pb', name: 'Plan Básico' }, signals: { plan_card: 2 }, last_feature: null, requested_at: null, contacted_at: null, updated_at: iso(2880) },
            ]))
            else response = respond(page([
                { id: 'd1', director_name: 'María Elvia Aguilar', director_email: 'maria@ejemplo.com', director_account: 'EJ-0500', director_phone: '4617654321', reward_months: 3, times_shared: 2, email_sent_at: iso(1440), joined_at: null, rewarded_at: null, created_at: iso(1440), invited_by: { id: 'c', name: 'Evangelina Caracheo', username: 'EJ-0201', phone: '4611112222' }, joined_user: null },
