...
const ClientCard = ({ data, loading }: Props) => {
    if (loading) {
        return (
            <div className="flex justify-center rounded-xl border border-slate-200/80 bg-white py-8">
                <Spinner />
            </div>
        )
    }
    ...
    if (!data?.identified || !data.client) {
        return (
            <div className="flex items-start gap-2.5 rounded-xl border border-slate-200/80 bg-white px-4 py-5 text-sm text-slate-500">
    ...
    return (
        <div className="grid h-max content-start gap-4 rounded-xl border border-slate-200/80 bg-white p-4">
            <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                    ...
                    <p className="truncate text-xs text-slate-500">{client.account}</p>
                </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-1.5">
                ...
                {client.plan && <Chip className="bg-violet-100 text-violet-700">{client.plan}</Chip>}
                {client.rank && <Chip className="border border-slate-200 text-slate-600">{client.rank}</Chip>}
            </div>

            <dl className="grid grid-cols-2 gap-3 rounded-lg bg-slate-50 p-3 text-xs">
                <div>
                    <dt className="text-slate-500">Pagos {payments.year}</dt>
                ...
                <div>
                    ...
                    <dd className="mt-0.5 font-semibold text-slate-900">{payments.pending_periods}</dd>
                </div>
                {payments.in_review_periods > 0 && (
                    <div className="col-span-2 rounded-md bg-amber-50 px-2 py-1.5">
                        <dt className="text-amber-700">Comprobante por validar</dt>
