...
const MoneyBlock = ({ current, previous }: Props) => {
    ...
    return (
        <div className="grid gap-4 rounded-2xl border border-border bg-card p-5 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
            <div>
                ...
                <p className="mt-1.5 flex items-center gap-1 text-xs text-muted-foreground">
                    {diff >= 0 ? (
                        ...
                        <ArrowDownRightIcon className="size-3.5 text-muted-foreground" />
                    )}
                    {money(Math.abs(diff))} {diff >= 0 ? "más" : "menos"} que {monthName(previous.period)}
                    <span className="text-muted-foreground">({money(previous.collected)})</span>
                </p>
            </div>
            ...
            <div className="sm:border-l sm:border-border sm:pl-5">
                ...
                <p className="mt-1 text-3xl font-bold tracking-tight text-red-600 dark:text-red-400 sm:text-4xl">
